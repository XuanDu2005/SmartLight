/**
 * Cart E2E — login → add → update → remove flow.
 */
...
describe('Cart (e2e)', () => {
  ...
  it('POST /v1/cart/items adds an item to the cart', async () => {
    ...
    const added = items.find(
      (i: { productVariantId?: string; variantId?: string }) =>
        (i.productVariantId ?? i.variantId) === variantId,
    );
  ...
  it('PATCH /v1/cart/items/:id updates the quantity', async () => {
    ...
    const updated = itemsAfter.find((i: { id: string }) => i.id === itemId);
  ...
  it('DELETE /v1/cart/items/:id removes the item', async () => {
    ...
    const stillThere = itemsAfter.find((i: { id: string }) => i.id === itemId);
