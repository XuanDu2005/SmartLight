...
describe('Orders (e2e)', () => {
    ...
    let variantId;
    let orderId;
    const email = (0, test_utils_1.uniqueEmail)('orders');
    beforeAll(async () => {
        ...
        if (!variant)
            throw new Error('No active variant for orders test');
    ...
    it('Full flow: cart → checkout → reserve → order', async () => {
        ...
        const checkout = await handle.request
            ...
            .send({})
            .expect(201);
        const sessionId = checkout.body.id ?? checkout.body.data?.id ?? checkout.body.checkoutSession?.id;
