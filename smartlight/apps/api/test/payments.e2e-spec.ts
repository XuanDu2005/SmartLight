/**
 * Payments E2E — mocked MoMo / VNPay / PayPal webhooks.
 *
 * The payments service verifies the webhook signature against the
 * gateway's secret. In tests we sign the payload with the SAME secret
 * (read from env) so the signature verification path is exercised end
 * to end. The gateway-side HTTP call is replaced by mocking nothing —
 * we hit the public webhook endpoint directly with a properly-signed
 * payload.
 */
...
describe('Payments (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    if (!order) {
      ...
      if (!variant) throw new Error('No active variant for payments test');
  ...
  it('VNPay webhook with valid signature is accepted', async () => {
    ...
    const signData = sortedKeys
      .map((k) => `${k}=${encodeURIComponent((vnp_Params as Record<string, string>)[k])}`)
      .join('&');
    const signature = crypto
