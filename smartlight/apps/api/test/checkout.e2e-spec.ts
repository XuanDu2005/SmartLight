/**
 * Checkout E2E — cart → create checkout session → reserve inventory.
 *
 * This test uses the existing service-layer inventory reservation logic
 * to verify the public contract. The shape of the checkout response
 * (totals, status) is allowed to be slightly different from production
 * because we're testing the API surface, not the snapshot internals.
 */
...
describe('Checkout (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    if (!variant) throw new Error('No active variant for checkout test');
